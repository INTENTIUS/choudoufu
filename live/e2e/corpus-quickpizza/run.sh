...
# And two additions, each for one stage and each in a file of its own,
# because the published shape has neither: day2_count declares a
# two-instance count ConfigMap (added at stage 9), and day2_crash declares
# a pair of ConfigMaps where the second reads the first's name (added at
# stage 10), so there is an apply of several objects with a real edge
# between them to interrupt. Both are destroyed with the rest.
#
...
#                  interrupted apply did create - the unrecovered run this
#                  stage exists to catch - and the recovery check must fail.
