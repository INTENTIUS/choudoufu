...
#
#   P2/P3  the world MOVES between the approval and the apply - the Labelbox S3 bucket's
...
#          the Labelbox IAM role is read back through the CLI.
...
# the Labelbox IAM role, the out-of-band move is on the Labelbox S3 bucket - so the refusal
...
if [ -z "${BREAK:-}" ] && [ -z "${BREAK_REMOVE:-}" ] && [ -z "${BREAK_GREEN:-}" ] \
  ...
  P_REVIEWED_ADDR="$WANT_ROLE_ADDR"
  ...
  # labelbox_iam_role it reaches exactly one taggable resource
  # ($WANT_ROLE_ADDR) - the module's other object, where it has one, is an
  ...
  # other module in this root alone, the Labelbox S3 bucket included.
  ...
  log "  edited one argument: labelbox_iam_role's common_tags now merge in Reviewed = \"yes\""
  ...
  P_REVIEWED_TAG="$(awsl iam list-role-tags --role-name "$ROLE_NAME" --query "Tags[?Key=='Reviewed'].Value | [0]" --output text)"
  [ "$P_REVIEWED_TAG" = "None" ] || [ -z "$P_REVIEWED_TAG" ] \
    || fail "the refused apply still wrote the reviewed change: $ROLE_NAME carries Reviewed=\"$P_REVIEWED_TAG\""
  printf '%s\n' "$P_REFUSAL" | head -12
  ...
  P_LANDED="$(awsl iam list-role-tags --role-name "$ROLE_NAME" --query "Tags[?Key=='Reviewed'].Value | [0]" --output text)"
  [ "$P_LANDED" = "yes" ] \
    || fail "the approved change did not land: $ROLE_NAME carries Reviewed=\"$P_LANDED\", want \"yes\""
  log "  the identical artifact applied (0 added, 1 changed, 0 destroyed) and $ROLE_NAME now carries Reviewed=yes, read via the AWS CLI"
  ...
  P_GONE="$(awsl iam list-role-tags --role-name "$ROLE_NAME" --query "Tags[?Key=='Reviewed'].Value | [0]" --output text)"
  [ "$P_GONE" = "None" ] || [ -z "$P_GONE" ] \
    || fail "the reviewed tag is still on $ROLE_NAME after the revert: \"$P_GONE\""
  ...
  gauntlet_stage plan_approval pass "one argument edited (labelbox_iam_role's common_tags gain Reviewed=yes), \"plan -out=approved.tfplan\" wrote a $P_PLAN_BYTES-byte stock-format plan file whose whole change set is one update on $P_REVIEWED_ADDR; the world then moved out of band ($BUCKET_NAME's hm_team tag, through the AWS CLI, never through choudoufu) and \"apply approved.tfplan\" refused with \"The approved plan no longer matches the live system\" at exit 3, classifying the drift under \"This apply would do, and the approved plan does not include:\" and naming both $P_MOVED_ADDR and the live $BUCKET_NAME it was computed against, with \"Exit status 3\" spelled out for a pipeline; nothing was applied - $ROLE_NAME still carried no Reviewed tag, read back through the AWS CLI rather than from the absence of an \"Apply complete!\" line. Inverted control on the same run (the shape live/smoke/scenarios/apply-what-was-approved.sh reasons out): with the tag put back and nothing else changed, the IDENTICAL file applied - 0 added, 1 changed, 0 destroyed - and $ROLE_NAME read back with Reviewed=yes, so the refusal is earned by the drift and not handed out to every plan file. BREAK_APPROVAL=1 asserts stage 12's own recorded Break line (apply the planfile after a mutation and expect success) and correctly fails"
  log ""
fi
