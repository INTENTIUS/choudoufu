...
#          notices AND that stock notices identically - and the apply must
#          refuse: exit 3, the named summary, the unapproved row printed by
#          address AND by the live VPC id it was computed against, and the
#          reviewed change still not landed when the default security
#          group's tags are read back through the CLI.
...
# module.vpc.aws_default_security_group.this["main"], the out-of-band move
# is on module.vpc.aws_vpc.this["main"] - so the refusal has an EXTRA row
# to name rather than a values-only disagreement about the same row.
#
# WHY THE DEFAULT SECURITY GROUP'S TAGS, and not one of the other 27
# instances: the reviewed edit has to reach exactly ONE instance and must
# not disturb a live id a later part of this script already captured (issue
# #903's second trap). aws_default_security_group.this is for_each'd over
# var.vpcs, which this crossing sets to the single "main" VPC, and NOTHING
# in the module references it - so a tags-only update on it proposes one
# in-place change and cascades nowhere, unlike the log group (a data source
# reads its arn, see the marker-noise filter above), the flow-logs role
# (PART D renames it and PART F force-replaces it), aws_eip.nat (PART D's
# live-mv target), aws_vpc_endpoint.s3 (PART E deletes its block) or the
# interface endpoints (PART G scales their for_each).
#
...
# never as a pass.
if [ -z "${BREAK:-}" ] && [ -z "${BREAK_STAGE5:-}" ] && [ -z "${BREAK_GREENFIELD:-}" ] \
   && [ -z "${BREAK_DAY2_RENAME:-}" ] && [ -z "${BREAK_DAY2_REMOVE:-}" ]; then
  gauntlet_begin_stage plan_approval
  log "=== PART P: plan, review, apply (the approval gate, live/GAUNTLET.md #12) ==="

  P_EST="$ESTATE/blueprints/landing-zone-basic"
  P_VPC_MAIN="$ESTATE/modules/networking/vpc/main.tf"
  P_REVIEWED_ADDR='module.vpc.aws_default_security_group.this["main"]'
  P_MOVED_ADDR='module.vpc.aws_vpc.this["main"]'
  ...
  # printed for $VPC_ID a few lines above.
  [ "$CHANGED_ADDRS" = "$P_MOVED_ADDR" ] \
    || fail "STAGE 5 saw $VPC_ID at [$CHANGED_ADDRS], not $P_MOVED_ADDR - PART P's out-of-band row would be named wrong"
  P_SG_ID="$(awsl ec2 describe-security-groups --filters "Name=vpc-id,Values=$VPC_ID" "Name=group-name,Values=default" --query 'SecurityGroups[0].GroupId' --output text)"
  [ -n "$P_SG_ID" ] && [ "$P_SG_ID" != "None" ] || fail "no live default security group found in $VPC_ID for the change under review"
  log "  under review: $P_REVIEWED_ADDR ($P_SG_ID); moving out of band: $P_MOVED_ADDR ($VPC_ID)"

  log "=== P1. the change under review: one argument, on the default security group ==="
  [ "$(grep -c 'default-sg"' "$P_VPC_MAIN")" = "1" ] \
    || fail "the vpc module no longer carries exactly one default-sg Name tag to edit - the corpus pin has moved"
  perl -pi -e 's/^(\s*)Name = ("[^"]*-default-sg")$/$1Name = $2\n$1Reviewed = "yes"/' "$P_VPC_MAIN"
  [ "$(grep -c 'Reviewed = "yes"' "$P_VPC_MAIN")" = "1" ] \
    || fail "the reviewed edit did not write exactly one Reviewed = \"yes\" tag"
  log "  edited one argument: the default security group's tags now carry Reviewed = \"yes\""

  P_PLAN_OUT="$(cd "$P_EST" && "$TOFU" plan -input=false -no-color -out=approved.tfplan 2>&1)"; P_PLAN_RC=$?
  [ "$P_PLAN_RC" -eq 0 ] || { printf '%s\n' "$P_PLAN_OUT" | tail -40; fail "plan -out exited $P_PLAN_RC"; }
  [ -f "$P_EST/approved.tfplan" ] || { printf '%s\n' "$P_PLAN_OUT" | tail -20; fail "plan -out wrote no file"; }
  ...
  P_PLAN_BYTES="$(wc -c < "$P_EST/approved.tfplan" | tr -d ' ')"
  ...
  P_GATE_RC=0
  P_GATE_OUT="$(cd "$P_EST" && "$TOFU" apply -input=false -no-color approved.tfplan 2>&1)" || P_GATE_RC=$?
  ...
  P_REVIEWED_TAG="$(awsl ec2 describe-tags --filters "Name=resource-id,Values=$P_SG_ID" "Name=key,Values=Reviewed" --query "Tags[0].Value" --output text)"
  [ "$P_REVIEWED_TAG" = "None" ] || [ -z "$P_REVIEWED_TAG" ] \
    || fail "the refused apply still wrote the reviewed change: $P_SG_ID carries Reviewed=\"$P_REVIEWED_TAG\""
  printf '%s\n' "$P_REFUSAL" | head -12
  ...
  P_OK_RC=0
  P_OK_OUT="$(cd "$P_EST" && "$TOFU" apply -input=false -no-color approved.tfplan 2>&1)" || P_OK_RC=$?
  [ "$P_OK_RC" = "0" ] \
  ...
  P_LANDED="$(awsl ec2 describe-tags --filters "Name=resource-id,Values=$P_SG_ID" "Name=key,Values=Reviewed" --query "Tags[0].Value" --output text)"
  [ "$P_LANDED" = "yes" ] \
    || fail "the approved change did not land: $P_SG_ID carries Reviewed=\"$P_LANDED\", want \"yes\""
  log "  the identical artifact applied (0 added, 1 changed, 0 destroyed) and $P_SG_ID now carries Reviewed=yes, read via the AWS CLI"

  log "=== P5. put the estate back where the rest of this script expects it ==="
  rm -f "$P_EST/approved.tfplan"
  perl -ni -e 'print unless /^\s*Reviewed = "yes"$/' "$P_VPC_MAIN"
  [ "$(grep -c 'Reviewed = "yes"' "$P_VPC_MAIN")" = "0" ] \
    || fail "reverting the reviewed edit did not remove the Reviewed tag from the module"
  [ "$(grep -c 'default-sg"' "$P_VPC_MAIN")" = "1" ] \
    || fail "reverting the reviewed edit disturbed the default security group's own Name tag"
  P_BACK_OUT="$(cd "$P_EST" && "$TOFU" apply -input=false -auto-approve -no-color 2>&1)"; P_BACK_RC=$?
  [ "$P_BACK_RC" -eq 0 ] || { printf '%s\n' "$P_BACK_OUT" | tail -40; fail "the revert apply failed"; }
  P_GONE="$(awsl ec2 describe-tags --filters "Name=resource-id,Values=$P_SG_ID" "Name=key,Values=Reviewed" --query "Tags[0].Value" --output text)"
  [ "$P_GONE" = "None" ] || [ -z "$P_GONE" ] \
    || fail "the reviewed tag is still on $P_SG_ID after the revert: \"$P_GONE\""
  ...
  P_FINAL_ADDRS="$(changed_addrs_excluding_markers <<< "$P_FINAL_OUT")"
  [ -z "$P_FINAL_ADDRS" ] \
    || { printf '%s\n' "$P_FINAL_OUT" | grep -E '^  # .+ (will be|must be)'; fail "the estate is not converged again after PART P: $P_FINAL_ADDRS"; }
  [ ! -f "$P_EST/terraform.tfstate" ] || fail "PART P left a state file behind - this estate must never keep local state"
  log "  reverted; the estate is converged again and PART D starts from where it would have"

  log ""
  log "PART P (plan, review, apply): PASS"
  gauntlet_stage plan_approval pass "one argument edited (module.vpc.aws_default_security_group.this[\"main\"]'s tags gain Reviewed=yes - a single for_each instance nothing else in the module references), \"plan -out=approved.tfplan\" wrote a $P_PLAN_BYTES-byte stock-format plan file whose whole change set is one update on $P_REVIEWED_ADDR; the world then moved out of band ($VPC_ID's Name tag, through the AWS CLI, never through choudoufu) and \"apply approved.tfplan\" refused with \"The approved plan no longer matches the live system\" at exit 3, classifying the drift under \"This apply would do, and the approved plan does not include:\" and naming both $P_MOVED_ADDR and the live $VPC_ID it was computed against, with \"Exit status 3\" spelled out for a pipeline; nothing was applied - $P_SG_ID still carried no Reviewed tag, read back through the AWS CLI rather than from the absence of an \"Apply complete!\" line. Inverted control on the same run (the shape live/smoke/scenarios/apply-what-was-approved.sh reasons out): with the tag put back and nothing else changed, the IDENTICAL file applied - 0 added, 1 changed, 0 destroyed - and $P_SG_ID read back with Reviewed=yes, so the refusal is earned by the drift and not handed out to every plan file. BREAK_APPROVAL=1 asserts stage 12's own recorded Break line (apply the planfile after a mutation and expect success) and correctly fails"
  log ""
fi
# ══════════════════════════════════════════════════════════════════════════
