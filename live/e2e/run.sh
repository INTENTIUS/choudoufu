...
if [ "$HAVE_LIVE_ESTATE" -eq 0 ]; then
  ...
  # 43 of the fixture's 44 declared instances materialize: the client-named
  ...
  # scope. The forty-fourth is the standing residue's unowned role, and it
  ...
  MATERIALIZED=$((44 - $(count_lines "$(unowned_omissions "$OUT")")))
  ...
  echo "  empty plan over the full estate ($MATERIALIZED/44 materialized; the rest omitted as unowned, with an adoption hint); $((STEP5_T1 - STEP5_T0))s"
