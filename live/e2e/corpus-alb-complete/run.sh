...
if [ -n "$NODE_RESOLVE" ]; then
  ...
  log ""
  gauntlet_stage test_plan fail "0 Error diagnostics, down from 1 (gauntlet issue #397, 2026-08-24) - live-plan COMPLETES for this estate for the first time, exit 0. Two generic language fixes cleared the last static-path refusal (module.alb.aws_lb_listener_certificate.this[\"ex-https/0\"].certificate_arn, Non-static identity argument): an explicit instScope threaded through internal/live/identity's structural decomposition so a for-expression NESTED inside another can read the outer loop variable, plus a new collection-through-a-value-variable case (v / v.attr / lookup(v,\"attr\",d) / try(v.attr,d)) resolved through the element's own expression; and forCondIncludesTolerant now decides any filter clause that reads only the STRUCTURE the author wrote, by binding the value variable to the element's own rebuilt skeleton (rebuildConstructor, unknown at every refused leaf) instead of enumerating length()/comparison spellings. Behind them, four Errors nothing had ever reached appeared and were fixed in the same unit (HANDOFF row 5): terraform-aws-modules/terraform-aws-acm's two aws_route53_record.validation instances take their name and type from an unapplied aws_acm_certificate, and aws_route53_record has no tags map, so the NEEDS_DISCOVERY/SIBLING_APPLY answer promised a marker sweep that could never find them - identity.RecordFallbackType, which six other call sites already consult, is now asked on the sibling-apply path too and both drop to the record rung, binding from the record live-import had already written (identity verified by value against route53 list-resource-record-sets - see 3d). STILL BLOCKED, and the wall is now a NON-EMPTY plan rather than a refusal: $PLAN_LINE. Three families remain, none of them this unit's scope: (1) 2 aws_acm_certificate_validation instances plan a CREATE - the type has no classic Importer and no marker surface, so projection can never read it back; it needs the record rung too, which RecordFallbackType's condition 0 (NotImportable) currently refuses on the grounds that a record exists to be imported back, a rule that has no answer yet for a type with no cloud object to import. (2) module.alb.aws_lb_listener.this[\"ex-oidc\"] and aws_lb_listener_rule.this[\"ex-cognito/ex-oidc\"] each show authenticate_oidc.client_secret being added: an argument the provider never echoes back, which HANDOFF says the record holds, so this is the residue half of the same foundation. (4) aws_cognito_user_pool.this shows email_sending_account defaulting and two empty nested blocks disappearing, which needs checking against the real Cognito API before it is called ours or floci's. FAMILY 3 IS FIXED (gauntlet issue #401 family 3, this unit): aws_instance.this/.other used to show a sensitive-marked ami with the value unchanged - a marks-only diff, gone now that Plan: went from 5 to 3 to change. ami = data.aws_ssm_parameter.al2.value, and aws_ssm_parameter's own "value" attribute is unconditionally Sensitive in the provider schema regardless of what it names; the CONFIG-evaluated side of a plan picks that mark up through the ordinary dynamic evaluator, but the projected PRIOR side (built by a live read, off the wire, which has nowhere to carry a mark) never did, and nothing re-applied it - Options.DataResults, the only thing that could resolve the VALUE, is scoped to what identity resolution demands, and nothing about an EC2 instance's identity needs its AMI parameter, so the value could never be seeded either. Fixed generically in internal/live/projection/build.go: dataSensitivePath recognizes a bare attr = data.<type>.<name>.<field> reference and checks that DATA SOURCE's own schema for Sensitive, needing no read and no DataResults entry at all; configuredAttrsSeed now asks this for every flat, non-computed-only, non-identity, non-tags attribute (previously only Computed-EXCLUDED attributes reached the check at all, which is what made the first attempt at this fix inert for aws_instance.ami specifically - Optional+Computed is its real shape). Reaches every flat attribute of every type in every provider that reads a data source's own schema-Sensitive attribute directly, not aws_instance/aws_ssm_parameter by name. configuredTagsSeed has the identical, still-open gap for the "tags" argument - not fixed here."
fi
