...
| `estate.chdf.hcl` | The live sidecar: `estate = "identity-cohort"` (same value as `locals.tf`'s `estate_tag`, see `estateTag`) plus a `record_store "local"`, so this cohort is a plannable estate and not only a marker-tagged fixture (issue #291). |
