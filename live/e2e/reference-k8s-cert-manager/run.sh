...
[ "$G_RECORDS" = "$TOTAL_N" ] || fail "the greenfield record store holds $G_RECORDS file(s) for $TOTAL_N instances; since #1211 every applied kubernetes_manifest records the metadata.labels and metadata.annotations keys its own manifest declared, so a root of $TOTAL_N manifests owes exactly $TOTAL_N records - and the lost-store control below is only a measurement if there is a full store to lose"
