...
| Marker path (more server IDs) | `aws_nat_gateway.main`, `aws_launch_template.app`, `aws_ebs_volume.data` | EC2 mints `nat-…`, `lt-…` and `vol-…` IDs; the template's name is client-chosen but the provider's identity schema requires the ID. Same slice (#20). |
...
| `network.tf` | VPC, subnets, security group, route table, internet gateway, route, route table associations, the per-rule security group pair and the private NAT gateway (#20's third slice). |
