...
admission table (`internal/live/lint/admission_generated.go`, mirrored by
`internal/live/identity`'s `table_generated.go`, the copy the sweep and
identity resolution read). Both are generated by `tools/row-gen -emit` and
carry a `DO NOT EDIT` header; a type is not added to them by hand.
...
Generated by `tools/limits-gen` from the three registries that define what
this fork can refuse. Do not edit the two spans below by hand; run `just
limits` instead.
...
Three registries feed it.

- `internal/live/lint` refuses a construct before anything is evaluated.
  Every rule here also has a hand-written entry above, and the table links
  to it.
...
- `internal/live/passthrough` is the class this fork does not author.
  ...
  `internal/configs`, from the address parser, or from HCL itself. They
  reach you as refusals like any other, and three of them are the largest
  blockers measured anywhere in this repository.

**What a pass-through refusal is telling you** is almost always the same
thing: something in an identity, a `count` or a `for_each` cannot be worked
out from `var`, `local`, `path` and `terraform` alone. That is the binding
rule of this whole mode. The specific wording varies because the specific
step that failed varies, not because the rules differ.
...
which is the mechanism behind the claim that a refusal cannot exist without
something here to read: `internal/live/check`'s
`TestEveryRefusalDocsRefIsResolvable` fails when one points at a heading
...
**How often.** Blocked 57 configurations in the measured corpus, at 1953 sites.
...
**How often.** Blocked 13 configurations in the measured corpus, at 25 sites.
...
**How often.** Blocked 11 configurations in the measured corpus, at 20 sites.
...
| `aws_appsync_domain_name_api_association` | `aws_api_gateway_domain_name` | no (report-only) |
| `aws_arczonalshift_zonal_autoshift_configuration` | `null_resource` | no (report-only) |
| `aws_autoscaling_group` | `aws_api_gateway_domain_name` | no (report-only) |
...
| `aws_glue_security_configuration` | `aws_api_gateway_domain_name` | no (report-only) |
...
| `aws_kinesis_resource_policy` | `null_resource` | no (report-only) |
...
`aws_arczonalshift_autoshift_observer_notification_status`,
`aws_athena_named_query`, `aws_cloudfront_cache_policy`,
`aws_cloudfront_continuous_deployment_policy`, `aws_cloudfront_key_group`,
`aws_cloudfront_origin_access_control`,
`aws_cloudfront_origin_access_identity`,
`aws_cloudfront_origin_request_policy`, `aws_cloudfront_public_key`,
...
`aws_networkmanager_link_association`, `aws_notifications_event_rule`,
