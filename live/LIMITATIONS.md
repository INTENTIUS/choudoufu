...
**RESOLVED.** The narrowing landed as ruling 5 (`the foundation-order ruling (#388)`,
...
(`the foundation-order ruling (#388)`) and
...
(`the foundation-order ruling (#388)`). The two settings are opposites
...
covered when #126 measured it):
<!-- limits-gen:begin residue-hard-count -->12 types / 23 attributes<!-- limits-gen:end residue-hard-count -->
are write-only (`aws_ssm_parameter.value_wo`,
`aws_db_instance.password_wo`, and the other `_wo` twins, the plugin
protocol forbids the provider ever returning their values), and
<!-- limits-gen:begin residue-soft-count -->60 types / 140 attributes<!-- limits-gen:end residue-soft-count -->
...
| - | - | projection | Live resource verified but not importable | error | `internal/live/projection` | "Live resource verified but not importable" |
...
#### Live resource verified but not importable
...
`aws_amplify_backend_environment`, `aws_amplify_domain_association`,
...
`aws_appflow_connector_profile`, `aws_apprunner_custom_domain_association`,
`aws_apprunner_default_auto_scaling_configuration_version`,
`aws_appstream_fleet_stack_association`, `aws_appstream_user`,
`aws_appstream_user_stack_association`, `aws_appsync_api_cache`,
...
`aws_cloudfront_response_headers_policy`, `aws_cloudfrontkeyvaluestore_key`,
`aws_cloudwatch_dashboard`, `aws_cloudwatch_event_api_destination`,
`aws_cloudwatch_event_archive`, `aws_cloudwatch_event_connection`,
`aws_cloudwatch_event_endpoint`, `aws_cloudwatch_event_permission`,
...
`aws_datapipeline_pipeline_definition`, `aws_datazone_asset_type`,
`aws_datazone_environment_blueprint_configuration`,
`aws_db_instance_role_association`, `aws_db_proxy_default_target_group`,
`aws_detective_member`, `aws_devopsguru_event_sources_config`,
`aws_devopsguru_resource_collection`, `aws_devopsguru_service_integration`,
`aws_directory_service_conditional_forwarder`,
`aws_directory_service_trust`, `aws_dynamodb_global_secondary_index`,
`aws_dynamodb_global_table`, `aws_dynamodb_kinesis_streaming_destination`,
`aws_dynamodb_resource_policy`, `aws_ebs_fast_snapshot_restore`,
`aws_ebs_snapshot_block_public_access`, `aws_ec2_allowed_images_settings`,
`aws_ec2_client_vpn_route`, `aws_ec2_local_gateway_route`,
...
`aws_emr_studio_session_mapping`, `aws_fis_target_account_configuration`,
`aws_fsx_s3_access_point_attachment`, `aws_glue_catalog_table`,
`aws_glue_catalog_table_optimizer`, `aws_glue_classifier`,
`aws_glue_data_catalog_encryption_settings`, `aws_glue_resource_policy`,
...
`aws_lakeformation_data_cells_filter`, `aws_lakeformation_lf_tag`,
...
`aws_lexv2models_bot_locale`, `aws_licensemanager_association`,
`aws_lightsail_bucket_resource_access`, `aws_lightsail_domain_entry`,
...
`aws_ram_resource_association`, `aws_rds_cluster_role_association`,
`aws_redshift_endpoint_access`, `aws_redshift_endpoint_authorization`,
`aws_redshift_logging`, `aws_redshiftserverless_custom_domain_association`,
`aws_resourcegroups_resource`, `aws_route`, `aws_route53_cidr_collection`,
...
`aws_route_table_association`, `aws_s3_account_public_access_block`,
...
`aws_sagemaker_servicecatalog_portfolio_status`, `aws_scheduler_schedule`,
...
`aws_sesv2_account_vdm_attributes`, `aws_sesv2_email_identity_policy`,
...
`aws_spot_datafeed_subscription`, `aws_sqs_queue_policy`,
`aws_ssm_maintenance_window_target`, `aws_ssm_patch_group`,
...
`aws_synthetics_group_association`, `aws_transfer_access`,
...
`aws_wafv2_web_acl_rule`, `aws_workmail_domain`,
...
| `aws_amplify_backend_environment` | `aws_amplify_app` | no (report-only) |
...
| `aws_apprunner_custom_domain_association` | `aws_apprunner_service` | no (report-only) |
| `aws_appsync_api_cache` | `aws_appsync_api` | no (report-only) |
| `aws_athena_prepared_statement` | `aws_athena_workgroup` | no (report-only) |
...
| `aws_connect_phone_number_contact_flow_association` | `aws_connect_phone_number` | no (report-only) |
...
| `aws_ec2_local_gateway_route` | `aws_ec2_local_gateway_route_table` | no (report-only) |
...
| `aws_fis_target_account_configuration` | `aws_fis_experiment_template` | no (report-only) |
...
| `aws_lambda_function_scaling_config` | `aws_lambda_function` | no (report-only) |
...
| `aws_lb_trust_store_revocation` | `aws_lb_trust_store` | no (report-only) |
...
| `aws_networkmanager_attachment_routing_policy_label` | `aws_networkmanager_core_network` | no (report-only) |
...
| `aws_notifications_organizational_unit_association` | `aws_notifications_notification_configuration` | no (report-only) |
| `aws_organizations_delegated_administrator` | `aws_organizations_account` | no (report-only) |
...
| `aws_ram_resource_association` | `aws_ram_resource_share` | no (report-only) |
| `aws_resourcegroups_resource` | `aws_resourcegroups_group` | no (report-only) |
...
| `aws_route53_vpc_association_authorization` | `aws_route53_zone` | no (report-only) |
| `aws_route53_zone_association` | `aws_route53_zone` | no (report-only) |
| `aws_route_table_association` | `aws_route_table` | no (report-only) |
| `aws_s3_bucket_accelerate_configuration` | `aws_s3_bucket` | no (report-only) |
| `aws_s3_bucket_analytics_configuration` | `aws_s3_bucket` | no (report-only) |
| `aws_s3_bucket_intelligent_tiering_configuration` | `aws_s3_bucket` | no (report-only) |
...
| `aws_s3_bucket_request_payment_configuration` | `aws_s3_bucket` | no (report-only) |
...
| `aws_sesv2_email_identity_policy` | `aws_sesv2_email_identity` | no (report-only) |
| `aws_sesv2_tenant_resource_association` | `aws_sesv2_tenant` | no (report-only) |
...
| `aws_ssoadmin_managed_policy_attachment` | `aws_ssoadmin_permission_set` | no (report-only) |
...
| `aws_transfer_access` | `aws_transfer_server` | no (report-only) |
...
| `aws_verifiedaccess_instance_trust_provider_attachment` | `aws_verifiedaccess_instance` | no (report-only) |
...
| `aws_vpc_route_server_propagation` | `aws_vpc_route_server` | no (report-only) |
| `aws_vpc_route_server_vpc_association` | `aws_vpc_route_server` | no (report-only) |
| `aws_vpc_security_group_vpc_association` | `aws_vpc` | no (report-only) |
| `aws_wafregional_web_acl_association` | `aws_wafregional_web_acl` | no (report-only) |
| `aws_wafv2_web_acl_association` | `aws_wafv2_web_acl` | no (report-only) |
...
**Total.** 173 types swept via a parent read.
...
`aws_accessanalyzer_archive_rule`, `aws_acmpca_policy`,
`aws_app_cookie_stickiness_policy`, `aws_appautoscaling_policy`,
`aws_appflow_connector_profile`,
`aws_apprunner_default_auto_scaling_configuration_version`,
`aws_appstream_fleet_stack_association`, `aws_appstream_user`,
`aws_appstream_user_stack_association`, `aws_appsync_domain_name`,
...
`aws_cloudwatch_log_subscription_filter`, `aws_cloudwatch_otel_enrichment`,
...
`aws_connect_lambda_function_association`,
`aws_connect_user_hierarchy_structure`, `aws_controltower_control`,
`aws_datazone_asset_type`, `aws_db_instance_role_association`,
`aws_db_proxy_default_target_group`, `aws_devopsguru_event_sources_config`,
`aws_devopsguru_resource_collection`, `aws_devopsguru_service_integration`,
`aws_directory_service_conditional_forwarder`,
`aws_directory_service_trust`, `aws_dynamodb_global_secondary_index`,
`aws_dynamodb_global_table`, `aws_dynamodb_kinesis_streaming_destination`,
...
`aws_kinesis_account_settings`, `aws_kinesis_resource_policy`,
`aws_kms_alias`, `aws_lakeformation_data_cells_filter`,
`aws_lakeformation_lf_tag`, `aws_lakeformation_lf_tag_expression`,
`aws_lambda_layer_version_permission`, `aws_lambda_permission`,
`aws_lambda_provisioned_concurrency_config`, `aws_launch_configuration`,
`aws_licensemanager_association`, `aws_lightsail_bucket_resource_access`,
`aws_lightsail_domain_entry`, `aws_lightsail_lb_attachment`,
`aws_lightsail_lb_certificate_attachment`, `aws_lightsail_static_ip`,
`aws_macie2_classification_export_configuration`,
...
`aws_ses_identity_policy`, `aws_ses_receipt_filter`, `aws_ses_receipt_rule`,
`aws_ses_receipt_rule_set`, `aws_ses_template`,
`aws_sesv2_account_vdm_attributes`, `aws_signer_signing_profile_permission`,
`aws_spot_datafeed_subscription`, `aws_ssm_resource_data_sync`,
