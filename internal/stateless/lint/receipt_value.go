...
// checkReceiptValueRule enforces stateless/RECEIPTS.md's Guard 2 ("Hash-only
...
func checkReceiptValueRule(mod *configs.Module, path addrs.Module, issues *[]Issue) {
	...
	for _, resource := range mod.ManagedResources {
				...
				*issues = append(*issues, Issue{
					...
					Detail: fmt.Sprintf(
						"%q is a receipt under the /tofu-receipts/ naming convention "+
							"(stateless/RECEIPTS.md), and a receipt is never a SecureString. Its value "+
		...
		*issues = append(*issues, Issue{
			...
			Detail: fmt.Sprintf(
				"%q is a receipt under the /tofu-receipts/ naming convention "+
					"(stateless/RECEIPTS.md), and its value must be evidently one of the two "+
