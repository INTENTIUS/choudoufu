...
type Result struct {
	...
	// stateless/MARKERS.md: the tag value that was matched and the tag value
	// that was written.
...
func Move(ctx context.Context, req Request) (*Result, tfdiags.Diagnostics) {
	...
	case !discovery.ValidEstateName(req.Estate):
		return res, diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Invalid estate name",
			fmt.Sprintf("A rename needs the estate's name, matching the tofu-estate marker grammar in stateless/MARKERS.md (a lowercase letter followed by letters, digits or hyphens, at most 128 characters). Got %q.", req.Estate),
		))
	...
	for _, m := range []struct {
		...
		if len([]rune(m.marker)) > discovery.MaxTagValue {
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("The address %s escapes to %d characters, over the %d-character AWS tag value limit, so no live resource can carry it as a tofu-address marker. See stateless/MARKERS.md.", m.addr, len([]rune(m.marker)), discovery.MaxTagValue),
			))
			continue
		}
		if !discovery.ValidMarkerAddress(m.marker) {
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("The address %s escapes to %q, which is not a well-formed tofu-address value under stateless/MARKERS.md. A marker written from it could never be matched back to this address.", m.addr, m.marker),
			))
		}
	}
	return diags
