...
type renameSource struct {
	...
	// stateless/MARKERS.md cannot round-trip. Such a marker still counts
...
func (b *renameBlock) ambiguity(ctx context.Context, c *classifier) RenameAmbiguity {
	...
	case single && !b.orphans[0].readable:
		amb.Detail = fmt.Sprintf(
			"One live %s carries the marker %q and one declared instance of %s is unclaimed, but that marker's instance key contains a '.' or a ':', which the escaping rule in stateless/MARKERS.md cannot turn back into an address. Writing the rename command would mean guessing where the key ends.",
