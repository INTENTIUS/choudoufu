...
func statelessBegin(
	...
	if settings.EstateSet && !discovery.ValidEstateName(settings.Estate) {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("The live block names estate %q, which does not match the tofu-estate marker grammar in stateless/MARKERS.md: a lowercase letter followed by lowercase letters, digits or hyphens, at most 128 characters.", settings.Estate),
...
//
// The order is the one internal/command/stateless_plan.go documents and for
...
func (r *statelessRunner) estateName(ctx context.Context, config *configs.Config) (string, tfdiags.Diagnostics) {
	...
	case 1:
		return "", diags.Append(&hcl.Diagnostic{
			...
			Detail: fmt.Sprintf(
				"The configuration stamps %s = %q, which does not match the marker grammar in stateless/MARKERS.md, so it cannot name this estate. Set a valid name with the live block's estate argument.",
