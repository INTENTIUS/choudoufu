...
// metadata.annotations keys it owns on a live kubernetes_manifest object,
// so that a key DELETED from the configuration can be proposed for
// removal instead of being invisible.
//
...
//
// Nil is not a silent degradation: the projection warns per instance
// whenever a manifest-shaped read had no hook to ask, and a
...
// be a guess, and a guess here reads as "you own nothing" and silently
// restores the defect. The caller turns the error into the warning that
// says the removal analysis did not run.
