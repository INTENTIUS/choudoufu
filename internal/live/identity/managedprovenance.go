...
func selectReferencedValue(val cty.Value, key addrs.InstanceKey, remaining hcl.Traversal) (cty.Value, bool) {
	...
	cur := val
	// val's own IsMarked test above does not carry over to cur by value -
	// internal/live/marksafe compares receivers as rendered text, and "cur"
	// is a different name from "val" even though they hold the same value
	// at this line. cur.LengthInt() in the IntKey branch below is one of
	// marksafe's 14 mark-unsafe cty methods, so it needs its own guard on
	// the name it is actually called through, not a comment explaining why
	// the earlier one should still apply. See TestNoUnprovenUnsafeCallSite.
	if cur.IsMarked() {
		return cty.NilVal, false
	}
	switch k := key.(type) {
