...
#
# aws_iam_group is the child throughout: a real DefaultTable row
...
# answer.
resource "aws_iam_group" "from_pet" {
  name = "svc-${random_pet.suffix.id}"
}

resource "aws_iam_group" "from_data" {
  name = "seeded-${terraform_data.seed.output}"
}

resource "aws_iam_group" "from_null" {
...
resource "aws_iam_group" "from_both" {
