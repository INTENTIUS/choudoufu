...
// EXISTENCE flavor of the receipts pattern (live/RECEIPTS.md, "Two
...
//
//	TF_FLOCI_TEST=1 go test ./internal/live/lifecycle/ -run TestReceiptExistenceFlavorAgainstFloci -v
//
...
// ordinary command surface, not the flag surface live/e2e/run.sh
...
func TestReceiptExistenceFlavorAgainstFloci(t *testing.T) {
	...
	// the same gap live/e2e/run.sh's step 3d works around for the full
...
// live/e2e/estate/receipts.tf's aws_ssm_parameter.demo_existence but
