...
// evaluator that can render an expression at each index in it. An
...
func countIndexDomainFor(ctx context.Context, mod *configs.Module, resource *configs.Resource, addr string) countIndexDomain {
	if resource.Count == nil || mod == nil || mod.StaticEvaluator == nil {
	...
	return countIndexDomain{ctx: ctx, eval: eval, subject: addr, n: n, ok: true}
}
