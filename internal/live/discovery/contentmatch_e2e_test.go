...
//
// aws_cloudfront_cache_policy is the real, generated binding this uses:
// Argument "name", CFN type AWS::CloudFront::CachePolicy, PropertyPath
// CachePolicyConfig.Name (tools/row-gen's contentMatchRoster, verified
...
func TestScanTypeContentMatch(t *testing.T) {
	binding, ok := identity.ContentMatchTypes["aws_cloudfront_cache_policy"]
	if !ok {
		t.Fatal("aws_cloudfront_cache_policy is not in identity.ContentMatchTypes; this test's whole premise depends on it")
	}

	cfg := loadConfig(t, filepath.Join("testdata", "contentmatch-e2e"))
	addr := mustAddr(t, "aws_cloudfront_cache_policy.x")
		...
		srv.listResources[binding.CFNType] = []ccResource{
			{
				...
				properties: map[string]any{
					...
					"CachePolicyConfig": map[string]any{"Name": "my-policy", "MinTTL": float64(1)},
				},
			},
			{
				// A second live cache policy with a DIFFERENT name must
				// never interfere with the match.
				identifier: "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee",
				properties: map[string]any{
					"Id":                "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee",
					"CachePolicyConfig": map[string]any{"Name": "unrelated-policy", "MinTTL": float64(60)},
				},
			},
		}
		server := srv.start()
		...
		srv.listResources[binding.CFNType] = []ccResource{
			{
				...
				properties: map[string]any{
					"Id":                "658327ea-f89d-4fab-a63d-7e88639e58f6",
					"CachePolicyConfig": map[string]any{"Name": "my-policy"},
				},
			},
			{
