...
//
//	TF_FLOCI_TEST=1 go test ./internal/live/discovery/ -run TestDiscoverAgainstFloci -v
//
...
func TestDiscoverAgainstFloci(t *testing.T) {
	...
	// filtered list goes out empty (live/e2e/estate/versions.tf). The
	...
	for _, s := range res.Scans {
		...
		case s.AccountID == "":
			t.Errorf("%s listed identities with no account ID: the provider resolved no account, so its owner-id filter went out empty (see live/e2e/estate/versions.tf)", s.TypeName)
		}
	}
