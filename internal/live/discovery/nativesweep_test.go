...
// exactly the cases the stale-state ruling's (#604)ays it should.
func TestNativeSweepNarrowsToEstateEvidence(t *testing.T) {
	...
	t.Run("a record store with no evidence of the type skips it", func(t *testing.T) {
		if listedNativeType(t, func(req *Request) { req.HintStore = nativeSweepStore(t, `aws_vpc.main`) }) {
			t.Errorf("%s was listed even though this estate's configuration and record store contain no instance of it and CollectUnclaimed was unset. That list call is the cost the stale-state ruling's (#604) ruling removes.", nativeLegType)
		}
	})
