...
choudoufu does not write receipts; it lints them. The value must be a hash or
constant and never a `SecureString`, nothing may reference a receipt's
...
`terraform_data` is for the dependency graph: ordering an apply, feeding
