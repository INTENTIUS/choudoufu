...
|---|---|---|
| [Claim 5: Recovery is a re-run, never surgery]({{< relref "/docs/claims/recovery-is-a-rerun" >}}) | An apply that crashed after a create, and a working copy with every local file deleted. Both end in a clean re-run. | `just smoke recovery-is-a-rerun` |
| [Claim 17: A record-only composite identity survives cache loss]({{< relref "/docs/claims/record-only-survives-cache-loss" >}}) | The cache gone and the record intact recovers; the record gone as well produces one named duplicate create, never a silent bind. | `just smoke record-only-survives-cache-loss` |
...
and is covered by
[the ownership policy matrix]({{< relref "/docs/use/ownership-policy" >}}).
...
lookup]({{< relref "/docs/use/resource-tiers" >}}) has the per-type table and
...
[`choudoufu live-import`]({{< relref "/docs/use/migrate" >}}) reads it, and it
...
- [Where things are stored]({{< relref "/docs/use/storage" >}}) - what the
  record store holds, per-backend, and who can read it
- [What you set up by hand]({{< relref "/docs/use/setup" >}}) - what has to
  exist before the first plan
- [Resource tier lookup]({{< relref "/docs/use/resource-tiers" >}}) - your own
  types, one by one
- [Migrate an existing estate]({{< relref "/docs/use/migrate" >}}) - the path
