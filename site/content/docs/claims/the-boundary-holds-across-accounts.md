...
1. `two accounts, one estate` - `sts:GetCallerIdentity` under each
   credential answers with a different account id. Then one apply over
   both provider configurations makes two log groups with the same name
   in the same region and two different account-qualified ARNs. Each
   carries the same `tofu-estate` and its own `tofu-address`, and each is
   **invisible to the other account's own listing**. Each account also
   holds a VPC of its own, and one record store holds both accounts'
   instances. Then a `-refresh=false` plan is empty, with the work
   attributed per account by the credential each request was signed
   with. The estate-wide tag index shows one fetch signed as each
   account and none unsigned (#957).
