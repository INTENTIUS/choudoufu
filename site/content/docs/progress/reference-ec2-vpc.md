...
|---|---|---|---|
| Cold deploy | pass | 1m17s | 5 resources from plain terraform, a real terraform.tfstate, zero markers |
| Migrate | pass | 57s | 5 of 5 verified, 5 stamped, 0 skipped |
| Replan from nothing | pass | 5s | post-adoption plan is empty; markers read back through the AWS CLI in part A |
| No-op apply | pass | 4s | no-op apply (0 added, 0 changed, 0 destroyed); tofu-estate-tagged object count unchanged at 5 |
| Drift and reconverge | pass | 7s | one object tampered, exactly aws_instance.main proposed, apply changed 1 and the tag reads back as configured |
| Rename | pass | 12s | moved block: aws_security_group renamed with zero churn (0 add, 1 change, 0 destroy), marker rewritten in place; live-mv: aws_internet_gateway renamed with zero churn, marker rewritten in place; stock oracle over the same two-resource rename on cold_deploy's own state also shows zero churn (0 add, 0 change, 0 destroy); both live ids unchanged, read via the AWS CLI |
...
| Change count (planned) | FAIL | 9s | choudoufu's scale-down plan does not destroy count_test[1] |
| Replace with create_before_destroy | pass | 31s | choudoufu: changing aws_instance.main's ForceNew ami argument proposed exactly one isolated instance replace at the same declared address (1 to add, 1 to destroy, nothing else), matching B1.8's own plan shape; applied cleanly; the old instance (i-ddb9110fec250d193) is confirmed terminated and the new instance (i-956c0bf9409f687b1) carries the marker, both via the AWS CLI; the local record store's record at the same address now names the new instance, not the terminated one; the next plan proposes no resource action. No BREAK=replace leg - see this section's own header comment (reusing corpus-security-group-complete's own finding from this same unit rather than re-measuring it here). |
...
Last run at commit `eff9afb400` on 2026-08-29T05:29:33Z, exit code 1, against emulator image `ghcr.io/lex00/floci@sha256:c55d74e13e96c8b132056677337dba0084bb0b427cb039be2dbf9a8b7efc0948`. Total run time 3m32.3s.
