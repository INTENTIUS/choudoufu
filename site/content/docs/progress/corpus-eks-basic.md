...
| Greenfield apply | FAIL | the greenfield apply failed - see the comment immediately above this call for the named root cause (a provider whose config depends on a data source only the same apply's own EKS cluster creation can resolve, a bootstrapping path no earlier stage in this script exercises through choudoufu) |
| Strict profile (planned) | not run |  |

Last run at commit `e064df4630` on 2026-08-25T14:37:22Z, exit code 1.
