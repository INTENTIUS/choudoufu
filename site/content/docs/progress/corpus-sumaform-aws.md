...
| Remove a block | FAIL | choudoufu does not propose destroying module.server.module.server.module.host.aws_instance.instance[0] when module.server's block is deleted |
| Change count (planned) | not run |  |
| Replace with create_before_destroy (planned) | not run |  |
| Crash between create and destroy (planned) | not run |  |
| Teardown (planned) | not run |  |
| Plan, review, apply (planned) | not run |  |
| Greenfield apply | FAIL | the greenfield replan is not empty |
| Strict profile (planned) | not run |  |

Last run at commit `2ce2eac21e` on 2026-08-25T04:41:37Z, exit code 1.
