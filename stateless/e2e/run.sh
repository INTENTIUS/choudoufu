...
# `live` block with no live-prefixed command anywhere in sight, and
# a receipt cycle (break an effect's memory out of band, watch the plan
# re-arm it). Nothing here is simulated or mocked: the emulator is a real AWS
# API surface, and every step is a real `choudoufu` binary built from this
# checkout.
#
...
#
# The estate fixture (stateless/e2e/estate/, task P0.1) is a separate
...
# stateless/e2e/estate is never written to; the copy applied against Floci
...
#   STATELESS_E2E_EXACTNESS  gates the drift-exact/removal-exact/receipt-cycle/
...
#   STATELESS_E2E_JSON=1     same as passing --json (see below).
#
# Flags:
...
#                     stateless/e2e/README.md for the parse contract.
...
ESTATE_SRC="$ROOT/stateless/e2e/estate"
BLOCK_SRC="$ROOT/stateless/e2e/estate-block"
...
# STATELESS_E2E_JSON=1 is the env-var spelling of --json (stateless/e2e/README.md).
JSON_MODE=0
EXPECT_PHASE=""
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "see stateless/e2e/README.md for the full contract"
      exit 0 ;;
    *) echo "unknown argument: $1 (supported: --json, --expect <phase>)" >&2; exit 2 ;;
  esac
done
[ "${STATELESS_E2E_JSON:-0}" = "1" ] && JSON_MODE=1
...
phase_of_step() {
  case "$1" in
    ...
    drift-exact|removal-exact|receipt-cycle|receipt-cycle-existence|lint-rejects) printf '5' ;;
    *) printf '' ;;
  esac
}
...
# The EXIT trap is registered before preconditions so a --json/STATELESS_E2E_
...
# untaggable-child boundary stateless/LIMITATIONS.md documents, showing up
...
# (internal/stateless/lifecycle), which prove the same claim directly against
...
STATELESS_E2E_EXACTNESS="${STATELESS_E2E_EXACTNESS:-1}"
...
echo "  STATELESS_E2E_EXACTNESS: $STATELESS_E2E_EXACTNESS"
...
if [ "$HAVE_LIVE_ESTATE" -eq 0 ]; then
  ...
  # 19 of the fixture's 20 declared instances materialize: the client-named
  ...
  # scope. The twentieth is the standing residue's unowned role, and it is
  # absent for a stated reason rather than silently — which is what the
  ...
  MATERIALIZED=$((20 - $(count_lines "$(unowned_omissions "$OUT")")))
  ...
  echo "  empty plan over the full estate ($MATERIALIZED/20 materialized; the rest omitted as unowned, with an adoption hint); $((STEP5_T1 - STEP5_T0))s"
  record_step "empty-plan-full" pass
fi

# ── 6. drift-exact ───────────────────────────────────────────────────────────
# P5.1's drift matrix (internal/stateless/lifecycle/
...
if [ "$HAVE_LIVE_ESTATE" -eq 0 ] || [ "$HAVE_LIVE_MV" -eq 0 ] || [ "$STATELESS_E2E_EXACTNESS" != "1" ]; then
  not_implemented "drift-exact" 5 "drift exactness is P5.1, gated on -estate + live-mv existing and STATELESS_E2E_EXACTNESS=1 (P5.2 flips the default to 1)"
else
  STEP6_T0=$(date +%s)
  ...
  # lifecycle/exactness_test.go and stateless/LIMITATIONS.md.
...
if [ "$HAVE_LIVE_ESTATE" -eq 0 ]; then
  ...
  # (views/stateless_plan.go's "  <type> <live-id>( (<display>))?" shape),
...
if [ "$HAVE_LIVE_ESTATE" -eq 0 ] || [ "$HAVE_LIVE_MV" -eq 0 ] || [ "$STATELESS_E2E_EXACTNESS" != "1" ]; then
  not_implemented "removal-exact" 5 "removal exactness is P5.1, gated on -estate + live-mv existing and STATELESS_E2E_EXACTNESS=1 (P5.2 flips the default to 1)"
else
  STEP8_T0=$(date +%s)
  COPY="$(mktemp -d)"
  cp -R "$MAIN/." "$COPY/"
...
# fixture (stateless/e2e/estate-block/, its own README explains why it is a
...
if [ "$HAVE_LIVE_BLOCK" -eq 0 ]; then
  ...
  # stateless_mode.go), and emptying the config hits the whole-block-
...
# ── 12. receipt-cycle ────────────────────────────────────────────────────────
# PE.3 + stateless/RECEIPTS.md: the estate's HASH-flavor receipt
...
if [ "$HAVE_LIVE_ESTATE" -eq 0 ] || [ "$STATELESS_E2E_EXACTNESS" != "1" ]; then
  not_implemented "receipt-cycle" 5 "the receipt-cycle demo needs full-estate live-plan (-estate probe, PE.3's receipt fixture) and P5.1's exactness work (STATELESS_E2E_EXACTNESS=1, P5.2 flips the default to 1)"
else
  STEP12_T0=$(date +%s)
...
if [ "$HAVE_LIVE_ESTATE" -eq 0 ] || [ "$STATELESS_E2E_EXACTNESS" != "1" ]; then
  not_implemented "receipt-cycle-existence" 5 "same gating as receipt-cycle: needs full-estate live-plan (-estate probe, RA.6's second receipt fixture) and P5.1's exactness work (STATELESS_E2E_EXACTNESS=1, P5.2 flips the default to 1)"
else
  STEP12B_T0=$(date +%s)
  EXISTENCE_PARAM="/tofu-receipts/stateless-e2e/demo-existence"
...
# ── 13. lint-rejects ─────────────────────────────────────────────────────────
# PE.1's missing half. That task built stateless/e2e/limits/ — one minimal
# configuration per banned or bounded construct — and the lint unit suite that
# walks it (internal/stateless/lint/limits_test.go), but the e2e step its own
...
echo "=== 13. lint-rejects — every limits fixture is refused by its own named rule ==="
if [ "$HAVE_LIVE_PLAN" -eq 0 ]; then
  not_implemented "lint-rejects" 1 "choudoufu live-plan does not exist yet (P1.4), and lint runs inside it"
else
  LIMITS_DIR="$ROOT/stateless/e2e/limits"
...
overlong-address:overlong-address"
  ...
  for LPATH in "$LIMITS_DIR"/*/; do
    ...
    in_list "$LDIR" "$LINT_LISTED" \
      || fail "lint-rejects" "stateless/e2e/limits/$LDIR is in neither table — add it to LINT_ENFORCED with its rule, or to LINT_TODO with why nothing catches it yet"
  done
  for LDIR in $LINT_LISTED; do
    [ -d "$LIMITS_DIR/$LDIR" ] \
      || fail "lint-rejects" "the tables name stateless/e2e/limits/$LDIR, which does not exist"
  done
  ...
  for LDIR in $LINT_TODO; do
    ...
    LINT_RULES="$(echo "$TF_OUT" | sed -n 's/^Rule: \([a-z-]*\)\..*$/\1/p' | sort -u | tr '\n' ' ')"
    [ -z "${LINT_RULES% }" ] \
      || fail "lint-rejects" "$LDIR: a lint rule now fires here ([${LINT_RULES% }]) — move it into LINT_ENFORCED and update stateless/LIMITATIONS.md and internal/stateless/lint/limits_test.go in the same change"
