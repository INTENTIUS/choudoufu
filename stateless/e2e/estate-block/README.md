...
A second, smaller estate, standing next to `stateless/e2e/estate/` (P0.1) for
...
(`stateless/e2e/run.sh`, step 2) needs the opposite: a stock `choudoufu apply`
...
Adding a `live` block to `stateless/e2e/estate/` would turn `standup`'s
...
coverage, which the main estate and `internal/stateless/lifecycle`'s
...
- **`aws_iam_role`** — `chant/test/floci-gaps.md` #5: `iam:GetRole` omits
  ...
  assertion (`stateless/e2e/run.sh`, step 11.3) a genuinely empty plan
...
fixture's `.tf` files (contrast `stateless/e2e/estate/`, where every
...
step 11 (`stateless/e2e/run.sh`) asserts exactly that: the main estate's
