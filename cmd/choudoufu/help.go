...
//
// binName is the binary's own invocation name (os.Args[0]'s base name in
// normal use), so the usage banner names whatever this binary was actually
// built and run as rather than a name hardcoded at compile time.
func helpFunc(binName string, commands map[string]cli.CommandFactory) string {
...
Usage: %s [global options] <subcommand> [args]
...
`, binName, listCommands(commands, primaryCommands, maxKeyLen), listCommands(commands, otherCommands, maxKeyLen))
