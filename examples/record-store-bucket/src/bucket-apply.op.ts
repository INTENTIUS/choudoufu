 ...
 * only destructive changes is wrong here in both directions: a change to the
 * bucket policy destroys nothing and can still lock every run out of its own
 * records, and a change to the KMS key destroys nothing and can still make
...
export default Op({
  ...
  phases: [
    ...
    phase("Approve", [
      gate("apply", {
        description:
          "Applying changes the bucket that holds an estate's identity. A policy or key change " +
          "destroys nothing and can still lock runs out of their records, or make existing " +
          "records unreadable.",
      }),
    ]),
