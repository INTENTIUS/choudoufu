...
1589 rendered identities across 515 configuration directories in under a
...
- `corpus-autoscaling-complete` - re-crossed for real 2026-08-21 at `dbc5ebf575`: stages 1 and 2 pass (68 added, 41 stamped, 27 skipped), stage 3 fails on exactly two diagnostics, both unchanged verbatim by `#346`'s fix. The first is the `aws_autoscaling_traffic_source_attachment.this["ex-alb"].identifier` one, and measuring it is what showed it is NOT the shape `#346` fixed: it refuses as `Non-static identity argument`, which is `stringValueIn`'s `!IsWhollyKnown` branch, so the value evaluated and came back unknown rather than the selection refusing. The unknown is the ALB target group's ARN arriving through a tolerant module-ARGUMENT rebuild - the value-shaped route, filed as its own item. The second is `Component.SoleElement` on a zero-element `prefix_list_ids`, deliberate per its own registry text. Numbers and both diagnostics in full: `live/corpus-crossing-manifest.json`.
- `corpus-vpc-complete` - re-crossed for real 2026-08-21 at `dbc5ebf575`: stages 1 and 2 pass (62 added, 40 stamped, 22 untaggable, three identities asserted by value through the AWS CLI). `#346`'s fix removed its sole stage-3 diagnostic for real - the run now gets past identity resolution into discovery, which it could not do before - and did **not** move the stage outcome, which stays 2 of 5. What it fails on now was hidden behind `#346` one stage earlier and is a different layer entirely: Cloud Control lists the account's DEFAULT DHCP options set (`dopt-default`) with no Tags and `GetResource` finds none either, so `Listed resource with no tags` aborts the sweep over an object this estate neither creates nor declares. Filed separately.
...
- `corpus-leynos-monitoring` - stages 1-2 pass for real. `test_plan` no longer blocks on `#352` (the stateless pipeline ignoring `-target`), which is fixed and merged; re-crossed at the fix, `live-plan` now resolves, discovers, stamps and plans over the targeted set. What blocks it now is `lex00/floci#93`, and it is a different label: both alarms render `- datapoints_to_alarm = 1 -> null` for an argument the module never sets, and stock reproduces it identically down to an isolated one-alarm reproduction with no choudoufu in the process. Label 1, "OpenTofu fails here too" - so this estate is parked on the emulator, not on the fork.
...
- `corpus-rds-complete-postgres`, `corpus-ecs-fargate` - both block on the VALUE-shaped half of what `#346` was filed as one wall, and measuring `#346`'s fix against all four estates is what split them apart. Their module output is consumed as a module-CALL ARGUMENT, so it travels `tolerantVariables`/`rebuildConstructor`/`moduleOutputValue`, which produces a `cty.Value`; a deferred parent read is a `ParentRef`, which is not one. `corpus-ecs-fargate` additionally applies a function to the deferred value (`element(split("/", var.cluster_arn), 1)`), which `identity.Formula` has no way to express at all. Both need a mechanism this repository does not have. See "What to do next."
...
### 1. `#346` landed, and measuring it against all four estates split one wall into two

`#346`'s fix is merged (`dbc5ebf575`). What it actually resolves, and what it does not, is worth reading before anyone picks up the remainder, because the issue was filed as one wall shared by four estates and it is two walls shared by none of them evenly.

**What landed.** An identity ARGUMENT that selects out of a `for_each` element bound as an EXPRESSION, and lands on a sibling's non-identity, schema-Computed attribute, now resolves `PARENT_DERIVED` and renders from the sibling's live object after marker discovery. Three things had to move, and the first was invisible to all three prior design passes: a one-element list construct inside the element is never narrowed on that route (`soleElementExpr` narrows over the ARGUMENT's expression, and the list is in the ELEMENT), so even `cidr_blocks = ["10.97.0.0/16"]` refused there as "string required, but have tuple". Then `lookup(each.value, "k", d)` needed recognising. Then `parentPart`'s deferred-read gate needed to admit a `ClassNeedsDiscovery` parent, which is Pass 1's proposal, reachable only once the first two moved.

**What refutes Pass 3.** Its "nothing routes a module-output reference reached through `eachValueSelect` into `moduleOutputValue`" is false: `selectStatic` already chases a bare `module.<call>.<output>` leaf, and `TestModuleOutputSiblingComputedBareOutput` is that leg passing with no new module-output plumbing at all. Pass 3 was reading wall (1) wearing a module output's diagnostic. Its own unfinished check - re-run with schemas populated - was also run: byte-identical, because wall (1) is decided before any schema is consulted.

**The soundness hazard `internal/live/projection/read.go:78-81` names is not reached, and not by luck.** It belongs to a design that makes a FIRST resolution pass non-fatal so a SECOND can read live values; this decides inside one ordinary pass, before discovery has run at all. No pass was added, resolution stays fatal, `classifyOrphans` is untouched, and `projection.PlanInstances`' count/for_each limitation never came into it. When the promise cannot be kept - discovery did not find the parent - the child is omitted with `ReasonParentUnavailable` and the plan proposes creating it, which is a missing marker rather than a wrong one. `internal/live/projection/parentdefer_test.go` is the negative control.

**Measured reach** (`tools/refusal-probe -schemas` over the same 252-configuration corpus tree before and after): refused sites 13860 -> 13823, resolved instances 4893 -> 4924, blocked entries 194 -> 192, `Not an identity attribute` 45 -> 10, and **11 distinct AWS types** across 6 third-party configurations from 4 organisations plus 2 in-repo cohorts - `aws_route` (7), `aws_security_group_rule` (13), `aws_cloudwatch_event_target` (5), `aws_ssoadmin_managed_policy_attachment` (3), the four `aws_api_gateway_*` types, `aws_ssoadmin_account_assignment`, `aws_ec2_transit_gateway_route`, `aws_vpc_endpoint_route_table_association`. Zero regressions: no entry gained a site or lost an instance, and `TestIdentityGolden` reports 0 identities changed, 12 added, 0 removed.

**What is left, and it is a different mechanism.** `corpus-rds-complete-postgres` and `corpus-ecs-fargate` consume the module output as a module-CALL ARGUMENT, which travels `tolerantVariables`/`rebuildConstructor`/`moduleOutputValue` - a route that produces a `cty.Value`, and a deferred parent read is a `ParentRef`, which is not one. `corpus-autoscaling-complete`'s remaining `#346`-looking diagnostic is the same route reached from the other side: it refuses as `Non-static identity argument`, i.e. the value evaluated and came back unknown. And `corpus-ecs-fargate` needs more than a deferred read even if that route carried one - it applies `element(split("/", var.cluster_arn), 1)` to the deferred value, and `identity.Formula` holds literals and `ParentRef`s with no way to express a transform. All three are filed together; none is a gap in what landed.

**And `corpus-vpc-complete` has a new wall behind the cleared one.** Its stage 3 now gets past identity resolution into discovery and fails there, on the ACCOUNT'S DEFAULT DHCP options set: "Cloud Control listed a `aws_vpc_dhcp_options` with no Tags in its Properties, and refining it with GetResource found none either ... Live resources: dopt-default." The estate neither creates nor declares that object. Filed separately.
...
- **`corpus-leynos-monitoring`, stages 1-2** (2026-08-20): the ninth OpenTofu-native estate, `leynos/df12-www`'s `modules/monitoring`. Cold-deploys and migrates for real. It is also the estate that found `#352` - the first crossing script to lean on `-target` at the live-plan stage rather than only at cold deploy - and its stage 3 is what the fix was measured against; see "Where the estates stand".
- **`#352` (2026-08-21):** `-target`/`-exclude` reached only the final core `Plan()` call. Everything the stateless pipeline does in front of a plan - identity resolution, the data-read phase, and through them discovery and stamping - walked the whole configuration, so a resource stock removed from the graph before evaluating it could refuse the run on its own identity arguments. The fix reads the surviving set off the plan graph's own `TargetingTransformer` (`tofu.Context.TargetedResources`) rather than re-deriving targeting in the live layer, because targeting includes a targeted resource's transitive dependencies and two implementations of that would drift into a projection missing something the plan still acts on. An out-of-scope block is still resolved - `internal/live/discovery`'s declared-address set is built from resolutions, and a marker whose address is not declared reads as an orphan - only its refusals are withheld.
