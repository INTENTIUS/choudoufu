...
var typeOverridesIdentity = map[string]typeOverride{
	...
	"aws_cognito_identity_pool_roles_attachment": {
		...
		Apply: func(g *generator, body *hclwrite.Body, addr resourceAddr) {
			poolRef := `"us-east-1:00000000-0000-0000-0000-000000000000"`
			if pool, ok := g.byType["aws_cognito_identity_pool"]; ok {
				poolRef = fmt.Sprintf("%s.id", pool)
			}
			body.SetAttributeRaw("identity_pool_id", exprTokens(poolRef))
