...
func TestModuleExamplesQueuedNotRated(t *testing.T) {
	entryType := func(name, origin string, blocked bool, sites int, refusals map[string]int) struct {
		Name     string         `json:"name"`
		Origin   string         `json:"origin"`
		Blocked  bool           `json:"blocked"`
		Refusals map[string]int `json:"refusals"`
		...
		return struct {
			...
			Sites    int            `json:"sites"`
			Modules  int            `json:"unresolved_modules"`
		}{Name: name, Origin: origin, Blocked: blocked, Sites: sites, Refusals: refusals}
	}
	...
	modules, unknown := buildModuleQueue(s)
	if len(unknown) != 0 {
		t.Fatalf("fixture uses refusals with no action: %v", unknown)
	}
