...
// TestBuildEvidenceOnlySchemaBucket_Partition pins the three-way split
// [buildEvidenceOnlySchemaBucket] reports: Covered (already promoted, read
// off the #428 provenance marker), NotCovered (schema present but the
// bucket stayed evidence-only, classified by [schemaGapClass]), and
// NoIdentitySchemaCount (no survey entry, or the survey serves no identity
// schema at all).
func TestBuildEvidenceOnlySchemaBucket_Partition(t *testing.T) {
	proposals := []proposal{
		{TFType: "aws_covered", Bucket: bucketClientNamed, ArgName: "name", ArgSource: argSourceIdentitySchemaEvidenceOnly},
		{TFType: "aws_gap_multi", Bucket: bucketEvidenceOnly},
		{TFType: "aws_gap_marker", Bucket: bucketEvidenceOnly},
		{TFType: "aws_no_schema", Bucket: bucketEvidenceOnly},
		{TFType: "aws_unrelated_client_named", Bucket: bucketClientNamed, ArgName: "x", ArgSource: argSourceCarveSeed},
	}
	survey := map[string]surveyEntry{
		"aws_covered":    {Path: surveyPathClientNamed, Identity: &surveyIdentity{RequiredForImport: []string{"name"}}},
		"aws_gap_multi":  {Path: surveyPathClientNamed, Identity: &surveyIdentity{RequiredForImport: []string{"a", "b"}}},
		"aws_gap_marker": {Path: "marker", Identity: &surveyIdentity{RequiredForImport: []string{"arn"}}},
		"aws_no_schema":  {Path: "moves to Ops"}, // Identity nil
	}

	b := buildEvidenceOnlySchemaBucket(proposals, survey)

	if b.CoveredCount != 1 || len(b.Covered) != 1 || b.Covered[0] != "aws_covered" {
		t.Errorf("Covered = %v, want [aws_covered]", b.Covered)
	}
	if b.NotCoveredCount != 2 {
		t.Fatalf("NotCoveredCount = %d, want 2 (%+v)", b.NotCoveredCount, b.NotCovered)
	}
	classes := map[string]string{}
	for _, e := range b.NotCovered {
		classes[e.Type] = e.Class
	}
	if classes["aws_gap_multi"] != "multi-attribute" {
		t.Errorf("aws_gap_multi class = %q, want multi-attribute", classes["aws_gap_multi"])
	}
	if classes["aws_gap_marker"] != "taggable-marker-path" {
		t.Errorf("aws_gap_marker class = %q, want taggable-marker-path", classes["aws_gap_marker"])
	}
	if b.NoIdentitySchemaCount != 1 {
		t.Errorf("NoIdentitySchemaCount = %d, want 1", b.NoIdentitySchemaCount)
	}
	// aws_unrelated_client_named is bucketClientNamed but NOT via #428's
	// provenance marker, and never appears in survey - must not be counted
	// anywhere in this bucket.
	wantTotal := b.CoveredCount + b.NotCoveredCount + b.NoIdentitySchemaCount
	if b.EvidenceOnlyTotal != wantTotal {
		t.Errorf("EvidenceOnlyTotal = %d, want %d", b.EvidenceOnlyTotal, wantTotal)
	}
	if b.EvidenceOnlyTotal != 4 {
		t.Errorf("EvidenceOnlyTotal = %d, want 4 (the unrelated client-named row must not be counted)", b.EvidenceOnlyTotal)
	}
}
