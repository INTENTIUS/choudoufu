...
// classifier-versus-ratified check in this repository was satisfied there, because the
...
// decides its veto, and [buildComparison] for the population whose
...
// live/rowgen-mismatches.json rather than by splitting the table into two
...
// emitPartition is the comparison measurement: which admitted types a fresh
...
// comparison -mismatches measures, then writes the two files
...
func runEmit(out, errOut *os.File, allowRetraction bool) error {
	...
	// exactly emittedRows' key set, split by the comparison verdict.
...
// contents by repo-relative path, plus the two comparison measurements the
...
func buildEmitFiles(ratified map[string]identity.TypeIdentity, proposals []proposal, annotations map[string]annotation, grammar map[string]importGrammarRow, survey map[string]surveyEntry, logical logicalSchemas, schemaFacts map[string]schemaFactEntry) (files map[string][]byte, identityPart, lintPart emitPartition, err error) {
	...
	cmp := buildComparison(rows, proposals, annotations)
	...
	// a fact that needs a ruling. -mismatches stays ungated on purpose:
	...
	// reason the comparison above reads rows instead of the
...
// comparison's verdict. A type absent from matched entirely
// (comparison.go's NotInMappedSet: no fresh proposal exists to
