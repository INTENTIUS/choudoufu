...
func cmdScalePatchSeconds(root string, args []string) error {
	...
	if len(stages) == 0 && *note == "" {
		return fmt.Errorf("scale-patch-seconds needs at least one -stage id=seconds or a -note")
	}
	...
	fmt.Printf("scale-patch-seconds: patched estate=%s target=%s scale=%d, unaccounted_seconds=%.1f (%s)\n",
		*estate, *target, *scale, *rec.UnaccountedSeconds, ScaleRecordsPath)
	return nil
}
