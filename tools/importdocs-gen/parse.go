...
// "if missing", or "by default" - the AWS provider's own conditional-free
// spelling of the same fact ("By default generated by Terraform."), used
// verbatim on 12 arguments across v6.59.0 (aws_autoscaling_group's `name`,
// aws_iam_role's sibling wording is the "if omitted" form but
// aws_cloudfront_public_key, aws_db_event_subscription,
// aws_docdb_event_subscription, aws_efs_file_system's `creation_token`,
// aws_elb, aws_emr_security_configuration, aws_neptune_event_subscription,
// aws_pinpoint_app, aws_qldb_ledger and both aws_signer_signing_profile*
// types all use it too) that went unread before this line existed: every
// one of those rows reached [projection.NodeResolver.ResolveResourceIdentity]
// with an absent, non-ServerAssignedIfAbsent component and refused
// "No source for this instance's identity" on the ordinary greenfield case
// of a *_prefix-style argument left blank, exactly what
// corpus-autoscaling-complete's greenfield stage caught for
// aws_autoscaling_group. Confirmed by a full sweep of the cached v6.59.0
// doc corpus before this change landed: every bullet the widened pattern
// newly matches, combined with serverAssignedGenRe, is one of the 12 rows
// named above - no unrelated "by default" bullet flips.
var serverAssignedOmitRe = regexp.MustCompile(`(?i)\bif\s+(omitted|missing|not\s+(specified|provided|set))\b|\bby\s+default\b`)
